...
<<<<<<< HEAD
Container engines and {product-title} must run on the Linux operating system. If you wish to
...
=======
`openshift` which is an all-in-one {prodcut-title} installation. The archives for all
