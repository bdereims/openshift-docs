...
Image stream tag::
A named pointer to an image in an image stream. An image stream tag is similar to a container image tag. See xref:image-stream-tag[Image Stream Tag] below.

Image stream image::
An image that allows you to retrieve a specific container image from a particular image stream where it is tagged.
