...
To solve this problem where the {product-title} cluster is using
xref:../architecture/networking/network_plugins.adoc#openshift-sdn[{product-title} SDN] as the cluster networking solution, there are two ways to achieve network access to the pods.
...
----
=======
xref:../admin_guide/high_availability.adoc#admin-guide-high-availability[*ipfailover*
documentation], you must now create a service account and add it to the
*privileged* SCC. First, create the *f5ipfailover* service account:
