...
7. Openvswitch and CNI plugin RPMs need to be hosted *on a HTTP server reachable from the {product-title} Node VMs* (http://websrv.cpod-ocp.az-demo.shwrfr.com in this example). Those files are included in NCP tar file that you have downloaded fron link:https://my.vmware.com/web/vmware/details?downloadGroup=NSX-T-PKS-240&productId=673[MyVMware.com].
