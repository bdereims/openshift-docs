...
To deploy the integrated container image registry, use the `oc adm registry` command from
...
To deploy the integrated container image registry, use the `oc adm registry` command as a
...
internal container image registry. It is a secure cloud storage manageable through
...
registry you are using, see xref:../../install_config/registry/extended_registry_configuration.adoc#s3-driver[S3 Driver Configuration].
...
[IMPORTANT]
====
The container image registry pod runs as user *1001*. This user must be able to write to
