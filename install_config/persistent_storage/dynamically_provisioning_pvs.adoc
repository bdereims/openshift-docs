...
[IMPORTANT]
====
Azure StorageClass is revised in {product-title} version 3.7. If you upgraded from a previous version, either:

 * specify the property `kind: dedicated` to continue using the Azure StorageClass created before the upgrade. Or,
 * add the location parameter (for example, `"location": "southcentralus",`)
 in the *_azure.conf_* file to use the default property `kind: shared`. Doing
