...
[NOTE]
Some sections of this document refer to OpenShift. {product-title} is
based on OpenShift technology.
endif::[]
...
ifdef::atomic-registry[]
Most `oc` commands run in the context of a
xref:../architecture/core_concepts/projects_and_users.adoc#projects[project]. The `oc login`
selects a default project during xref:basic-setup-and-login[initial setup] to
...
After you have xref:get_started_cli.adoc#basic-setup-and-login[logged in],
explore administrator xref:admin_cli_operations.adoc#cli-reference-admin-cli-operations[CLI operations].
