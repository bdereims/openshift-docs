...
</div><details><summary><span title="(object) Data contains the secret data. Each key must consist of alphanumeric characters, &#39;-&#39;, &#39;_&#39; or &#39;.&#39;. The serialized form of the secret data is a base64 encoded string, representing the arbitrary (possibly non-string) data value here. Described in https://tools.ietf.org/html/rfc4648#section-4">data</span>:
...
</div></details><details><summary><span title="(object) stringData allows specifying non-binary secret data in string form. It is provided as a write-only convenience method. All keys and values are merged into the data field on write, overwriting any existing values. It is never output when reading from the API.">stringData</span>:
</summary><div style="margin-left:13px;">  <span title="(string)">[string]</span>:
</div></details><div style="margin-left:13px;"><span title="(string) Used to facilitate programmatic handling of secret data.">type</span>:
</div>
</pre>
++++
...
$ curl -k \
    ...
    https://$ENDPOINT/api/v1/secrets <<'EOF'
{
  "kind": "Secret",
  "apiVersion": "v1",
  ...
...
|200 OK|v1.Secret
<<<<<<< HEAD
|201 Created|v1.Secret
|202 Accepted|v1.Secret
=======
>>>>>>> 6bcfc60496ce2d67bb2988b30404317813a382f5
|401 Unauthorized|
|===
...
==== HTTP request
----
POST /api/v1/namespaces/$NAMESPACE/secrets HTTP/1.1
Authorization: Bearer $TOKEN
Accept: application/json
Connection: close
Content-Type: application/json'
...
==== HTTP request
----
GET /api/v1/namespaces/$NAMESPACE/secrets/$NAME HTTP/1.1
Authorization: Bearer $TOKEN
Accept: application/json
Connection: close
----
...
$ curl -k \
    -H "Authorization: Bearer $TOKEN" \
    -H 'Accept: application/json' \
    https://$ENDPOINT/api/v1/namespaces/$NAMESPACE/secrets/$NAME
----
...
==== HTTP request
----
GET /api/v1/namespaces/$NAMESPACE/secrets HTTP/1.1
Authorization: Bearer $TOKEN
Accept: application/json
Connection: close
----
...
$ curl -k \
    -H "Authorization: Bearer $TOKEN" \
    -H 'Accept: application/json' \
    https://$ENDPOINT/api/v1/namespaces/$NAMESPACE/secrets
----
...
$ curl -k \
    -H "Authorization: Bearer $TOKEN" \
    -H 'Accept: application/json' \
    https://$ENDPOINT/api/v1/watch/namespaces/$NAMESPACE/secrets/$NAME
----
...
$ curl -k \
    -H "Authorization: Bearer $TOKEN" \
    -H 'Accept: application/json' \
    https://$ENDPOINT/api/v1/watch/secrets
----
...
=== Watch all Secrets in a namespace
Watch individual changes to a list of Secret
...
$ curl -k \
    -H "Authorization: Bearer $TOKEN" \
    -H 'Accept: application/json' \
    https://$ENDPOINT/api/v1/watch/namespaces/$NAMESPACE/secrets
----
...
=== Update a Secret in a namespace
Replace the specified Secret

==== HTTP request
----
PUT /api/v1/namespaces/$NAMESPACE/secrets/$NAME HTTP/1.1
Authorization: Bearer $TOKEN
Accept: application/json
Connection: close
Content-Type: application/json'
...
$ curl -k \
    ...
    -H 'Accept: application/json' \
    -H 'Content-Type: application/json' \
    https://$ENDPOINT/api/v1/namespaces/$NAMESPACE/secrets/$NAME <<'EOF'
{
  "kind": "Secret",
  "apiVersion": "v1",
  ...
...
=== Patch a Secret in a namespace
Partially update the specified Secret

==== HTTP request
----
PATCH /api/v1/namespaces/$NAMESPACE/secrets/$NAME HTTP/1.1
Authorization: Bearer $TOKEN
Accept: application/json
Connection: close
...
==== HTTP request
----
DELETE /api/v1/namespaces/$NAMESPACE/secrets/$NAME HTTP/1.1
Authorization: Bearer $TOKEN
Accept: application/json
Connection: close
Content-Type: application/json'
...
==== HTTP request
----
DELETE /api/v1/namespaces/$NAMESPACE/secrets HTTP/1.1
Authorization: Bearer $TOKEN
Accept: application/json
Connection: close
----
